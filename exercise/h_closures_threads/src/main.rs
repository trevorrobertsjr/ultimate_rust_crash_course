...
fn main() {
    ...
    //
    // "move" keyword explicitly states taking ownership of my_vector.
    // RESEARCH: when do you not use "move"?
...
// unbounded means will take as many values as possible before runs out of memory
   ...
   // Challenge: Make two child threads and give them each a receiving end to a channel.  From the
    ...
    let handle_c = thread::spawn(move|| {
        for msg in my_rx {
            println!("Main thread: Thread C Received {}", msg);
        }

    });
    let handle_d = thread::spawn(move|| {
        for msg in my_rx2 {
            println!("Main thread: Thread D Received {}", msg);
        }
    });
    ...
    drop(my_tx); // close the channel so the receiving ends will break their loops and end
